  ...
  <section class="tokens-list relative z-10">
    ...
    <template v-else>
      <ul class="bg-black text-white">
        <li v-for="(token, index) in tokens" :key="token.tokenId + '_' + index" class="flex flex-wrap md:flex-nowrap min-h-9 justify-between" :class="{'opacity-30': isCaptured(index)}">
          <!-- token link -->
          <div v-if="demoAmount">{{ index }}</div>
          <a :href="store.getters.openSeaLink({tokenId: token.tokenId})" target="_blank" rel="noopener noreferrer" class="text-neutral-500 order-1 pl-2.5 pr-1.5 py-2.5 block">
            #{{ token.index }}
          </a>

          <a :href="store.getters.openSeaLink({tokenId: token.tokenId})" target="_blank" rel="noopener noreferrer" class="order-3 px-2.5 pb-2.5 md:py-2.5 md:flex-1 text-left mouse:hover:bg-[rgba(255,255,255,0.1)] flex gap-3">
          ...
          <a :href="store.getters.etherscanLink({tokenId: token.tokenId})" target="_blank" rel="noopener noreferrer" class="order-2 md:order-last flex-1 md:flex-none pl-3 pr-2.5 py-2.5 flex min-w-0 md:min-h-9 items-start justify-end text-right mouse:hover:bg-[rgba(255,255,255,0.1)]">
            <template v-if="isCaptured(index)">
              <div>CAPTURED BY <Addr :address="token.owner" class="underline" /></div>
            </template>
